...
/**
  ...
  * Used http://coffeescript.org/ to convert coffeescript to javascript
   */

 var Match, calculate_operations, consecutive_where, create_index, diff, find_match, find_matching_blocks, 
 html_to_tokens, is_end_of_tag, is_start_of_tag, is_tag, is_whitespace, isnt_tag, op_map, recursively_find_matching_blocks, 
 render_operations, wrap;
 ...
 wrap = function(tag, content) {
     ...
     while (true) {
         ...
         if (non_tags.length !== 0) {
            if(tag == "ins"){
              rendering += `<${tag} style=\"background-color: #8aff97; text-decoration: none;\">${non_tags.join('')}</${tag}>`;
            }
            else if(tag == "del"){
              rendering += `<${tag} style=\"background-color: #ff5c5c;\">${non_tags.join('')}</${tag}>`;
            }
            else {
